package com.example.facedetectionproject
    ...
    fun detectFace(bitmap: Bitmap){
        ...
        val result = detector.process(image)
            .addOnSuccessListener { faces ->
                ...
                for(face in faces){
                    resultText += "Face Number : $i" +
                            "\nSmile : ${String.format("%.2f", (face.smilingProbability ?: 0f) * 100)}%" +
                            "\nLeft Eye Open : ${String.format("%.2f", (face.leftEyeOpenProbability ?: 0f) * 100)}%" +
                            "\nRight Eye Open : ${String.format("%.2f", (face.rightEyeOpenProbability ?: 0f) * 100)}%"
                    i++
                }
                if(faces.isEmpty()){
            ...
            .addOnFailureListener { e ->
                // Task failed with an exception,our face isnot  successfully detected
